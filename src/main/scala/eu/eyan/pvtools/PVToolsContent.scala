...
object PVToolsContent {
	val TITLE = "Photo and video import"
}

class PVToolsContent {
    /**
...
/***************************  UI  ***************************************************/
  ...
  def parentWindow = Option(SwingUtilities.windowForComponent(panel))
  importPaths.subscribe(v => parentWindow.foreach(_.resizeAndBack)) // this resizeAndBack should be handled better! in the multi class
  ...
  filesToResize.withLatestOf(params$).subscribe(filesParams => execute(filesParams._1.par.foreach(resizeFile(filesParams._2))))
  ...
  private def checkFilesToImport(params: Params): List[File] = {
    ...
    if (params.importPaths.importPaths.size < 1) { alert("Set at least one import dir please."); List() }
      ...
      if (filesToImport.nonEmpty) parentWindow.map(_.asInstanceOf[JFrame]).foreach(_.state_Normal.visible.toFront)
